...
class JiraIntegration(JiraTools):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.register(self.create_security_ticket)
        self.register(self.update_ticket_with_findings)
        self.register(self.get_stale_tickets)
        self.register(self.get_security_ticket_details)
        ...
        if status:
            try:
                transition_result = self.transition_issue(
                    issue_key=ticket_id, transition=status
                )
        ...
        if add_labels:
            try:
                ...
                update_result = self.update_issue(
                    issue_key=ticket_id, fields={"labels": updated_labels}
                )
    ...
    def get_stale_tickets(
        self, hours_old: int = 24, status_filter: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for issue in search_result.get("issues", []):
                ...
                try:
                    ...
                    age_hours = (
                        datetime.now() - updated_time.replace(tzinfo=None)
                    ).total_seconds() / 3600
                except:
